...
export default function LeadTable({
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5" id="leads-table-container">
      ...
      {/* Table presentation for Desktop / Tablet View */}
      ...
      {/* Mobile Presentation - Cozy Stacked Card List */}
      <div className="block md:hidden space-y-4">
        {filteredLeads.length === 0 ? (
          <div className="text-center py-12 px-4 bg-slate-50 rounded-xl border border-dashed border-slate-200">
            <p className="font-bold text-slate-700 text-sm">No Extracted Leads Found</p>
            <p className="text-xs text-slate-400 mt-1 max-w-xs mx-auto">
              Run an OCR Scan on the simulator panel on the left or change active filters, and the structured leads will land right here!
            </p>
          </div>
        ) : (
          filteredLeads.map(lead => (
            <div key={lead.id} className="bg-slate-50/50 p-4 rounded-xl border border-slate-100 space-y-3.5 shadow-xs">
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="min-w-0">
                    <h4 className="font-bold text-slate-900 text-sm truncate">{lead.parentName}</h4>
                    <span className="text-[11px] text-slate-500 block truncate">Student: {lead.childName || 'N/A'}</span>
                  </div>
                </div>
                <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold shrink-0 self-start ${getStatusBadge(lead.status)}`}>
                  {lead.status}
                </span>
              </div>

              <div className="grid grid-cols-2 gap-2 text-[11px] bg-white p-2 text-slate-700 rounded-lg border border-slate-100">
                <div className="flex items-center gap-1.5 font-medium">
                  <GraduationCap className="w-3.5 h-3.5 text-slate-405 shrink-0" />
                  <span className="truncate">{lead.classGrade || 'N/A'}</span>
                </div>
              ...
              <div className="space-y-1.5 text-xs text-slate-600 bg-white p-3 rounded-lg border border-slate-100">
                {lead.email ? (
                  <a href={`mailto:${lead.email}`} className="flex items-center gap-2 text-indigo-600 hover:underline font-semibold break-all">
                    <Mail className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                ...
                {lead.address && (
                  <div className="flex items-start gap-2 text-[11px]">
                    ...
                    <span className="leading-tight text-slate-500">{lead.address}</span>
                  </div>
                )}
              </div>
              ...
              <div className="pt-2 border-t border-slate-100 flex flex-wrap items-center justify-between gap-2.5">
                <div className="flex items-center gap-1">
                  <span className="text-[10px] text-slate-400 font-bold mr-1">Folder:</span>
                  <select
                    title="Move Lead Folder"
                    value={lead.folderId}
                    onChange={e => onMoveLead(lead.id, e.target.value)}
                    className="px-2 py-1 text-[11px] rounded-lg border border-slate-200 bg-white hover:border-slate-300 focus:outline-none transition-colors font-bold text-slate-700"
                  >
                    <option value="all">📁 Move (All Pool)</option>
                    {folders.map(f => (
                      <option key={f.id} value={f.id}>
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    onClick={() => handleStartEdit(lead)}
                    className="p-1 px-2.5 rounded-lg bg-slate-100 hover:bg-indigo-50 text-slate-650 hover:text-indigo-600 transition-colors text-xs font-bold flex items-center gap-1"
                    title="Edit lead"
                  >
                  ...
                  <button
                    onClick={() => onDeleteLead(lead.id)}
                    className="p-1 px-2 rounded-lg bg-slate-100 hover:bg-rose-50 text-slate-650 hover:text-rose-600 transition-colors"
                    title="Delete lead"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
                  </button>
